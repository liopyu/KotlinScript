...
public class PathManager {
	...
	private static String extractRoot(URL resourceURL, String resourcePath) {
		...
		if ("file".equals(protocol)) {
			...
			String paths = splitJarUrl(resourceURL.getFile());
			if (paths != null) {
				resultPath = paths;
			}
			...
			if (!jarPath.startsWith("file")) {
				...
				try {
					...
					try {
						...
						throw new IllegalArgumentException("URL='" + parsedUrl.toString() + "'", var10);
					}
					...
					if (jarPath.startsWith("://")) {
						...
						return !jarPath.isEmpty() && jarPath.charAt(0) == ':' ? jarPath.substring(1) : jarPath;
					}
				}
			}
		}
